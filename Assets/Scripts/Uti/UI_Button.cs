...
public class UI_Button : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    public Image hoverBehaviour_Image_body;
    ...
    public void SetHoverBehaviourType(HoverBehaviour hoverBehaviourType)
    {
        ...
        switch (hoverBehaviourType)
        {
            case HoverBehaviour.Change_Color:
                hoverBehaviourFunc_Enter = delegate () { hoverBehaviour_Image.color = hoverBehaviour_Color_Enter; hoverBehaviour_Image_body.color = hoverBehaviour_Color_Enter; button.GetComponent<RectTransform>().anchoredPosition += new Vector2(100f,0); };
                hoverBehaviourFunc_Exit = delegate () { hoverBehaviour_Image.color = hoverBehaviour_Color_Exit; hoverBehaviour_Image_body.color = hoverBehaviour_Color_Exit; button.GetComponent<RectTransform>().anchoredPosition += new Vector2(-100f, 0); };
                break;
            ...
            case HoverBehaviour.Change_SetActive:
                ...
                break;
            case HoverBehaviour.Change_Pos:
                hoverBehaviourFunc_Enter = delegate () { button.GetComponent<RectTransform>().anchoredPosition += new Vector2(100f, 0); };
                hoverBehaviourFunc_Exit = delegate () { button.GetComponent<RectTransform>().anchoredPosition += new Vector2(-100f, 0); };
                break;
            case HoverBehaviour.Change_Pos_In:
                hoverBehaviourFunc_Enter = delegate () { button.GetComponent<RectTransform>().anchoredPosition += new Vector2(-100f, 0); };
                hoverBehaviourFunc_Exit = delegate () { button.GetComponent<RectTransform>().anchoredPosition += new Vector2(+100f, 0); };
                break;
            case HoverBehaviour.Change_Scale:
                hoverBehaviourFunc_Enter = delegate () { button.GetComponent<RectTransform>().localScale = new Vector2(1.2f, 1.2f); };
                hoverBehaviourFunc_Exit = delegate () { button.GetComponent<RectTransform>().localScale = new Vector2(1f, 1f); };
                break;
        }
    }
